...
Item {
  ...
  function updateCommand(kind, agentIds) {
    // Let Claude Code renew expiring logins before the collectors read them.
    var helper = decodeURIComponent(String(Qt.resolvedUrl("refresh_usage.py")).replace(/^file:\/\//, ""))
    var command = ["python3", helper]
